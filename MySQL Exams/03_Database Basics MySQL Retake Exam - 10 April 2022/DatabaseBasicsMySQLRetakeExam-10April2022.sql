...
CREATE FUNCTION `udf_actor_history_movies_count`(`full_name` VARCHAR(50))
    ...
    DETERMINISTIC
BEGIN
    DECLARE `space_place` INT;
    SET `space_place` := LOCATE(' ', `full_name`);
    RETURN (SELECT COUNT(*)
            FROM `actors` AS `a`
            ...
            WHERE `a`.`first_name` = SUBSTRING(`full_name`, 1, `space_place` - 1)
              AND `a`.`last_name` = SUBSTRING(`full_name`, `space_place` + 1)
              AND `g`.`name` = 'History');
END $$
DELIMITER ;
